...
** EVIL NERD Commenter
 ...
 #+END_SRC
** TODO edit-server (for chrome interaction)
 #+BEGIN_SRC emacs-lisp :results none
   ;; (require 'edit-server)
   ;;  (edit-server-start)
...
**** org editing
#+BEGIN_SRC emacs-lisp xq:results none
(global-set-key
 (kbd "C-M-o")
...
#+END_SRC
   ...
   ("d" org-timestamp-select  "time selector" :color blue)
   ("a n" org-timestamp-now   "time selector" :color blue)
   ("a i" z-insert-date   "ins date" :color blue)
...
**** dired omit 
Omitting a file means removing it from the directory listing. Omitting is useful for keeping Dired buffers free of ë°∞uninterestingë°± files (for instance, auto-save, auxiliary, backup, and revision control files) so that the user can concentrate on the interesting files.
