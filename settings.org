...
** where is the .folder file located
#+BEGIN_SRC emacs-lisp :results none
(setq wl-folders-file "~/.emacs.d/.folders")
#+END_SRC
** smtp config (to send emails)
#+BEGIN_SRC emacs-lisp :results none
...
#+BEGIN_SRC emacs-lisp :results none
#+END_SRC
** make the inbox "sticky"
he buffer of the ‘sticky summary’ does not killed by typing q.
...
The mime buttons is what shows up in the beginning and end of your messages as [1 <text/plain; iso-8859-1 (quoted-printable)>]. You can toggle on/off with C-c m (in summary buffer) or: 
$Note$ that turning this to nil (off) will make extracting attachments hard
#+BEGIN_SRC emacs-lisp :results none
;(setq mime-view-buttons-visible nil) 
#+END_SRC
** resort the summary buffer by date
...
** Look and feel 
*** left pane
this will create a sticky left pane 
#+BEGIN_SRC emacs-lisp :results none
(setq 
...
#+END_SRC
*** TODO Faces
Settings for "face" must be written in ~/.wl, because you can’t write them in .emacs (if you write it to .emacs, you’ll get an error). See Highlights.
-seems to work as part of leauven theme?
...
#+END_SRC
*** what name to give to the root tree in folder mode (folder view)
#+BEGIN_SRC emacs-lisp :results none
(setq wl-folder-desktop-name "Gmail")
#+END_SRC
** org integration
   :PROPERTIES:
   :ID:       a8c344b2-632e-4760-8bfc-ca411b22c5ad
   :END:
first you need to use the org-wl contib package (make sure org contrib is in your path)
#+BEGIN_SRC emacs-lisp :results none
(require 'org-wl)
#+END_SRC

then add to the capture templates>> [[id:2bf19d27-a0af-4e4c-91f1-9723a2dedf18][Capture templates]]  a relevant wl-email capture
* Extensions
** Xha menu dirs
#+BEGIN_SRC emacs-lisp :results none
