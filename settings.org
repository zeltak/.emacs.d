...
(defhydra hydra-unicode (:color blue)
  "unicode"
  (">"   (insert-char ?) "úÚ¢˚") ; C-x 8 RET 261b RET, pointing hand
  ("\\"  (insert-char ?úÚ°Ó) "úÚ°Ó") ; C-x 8 RET 9646 RET, black vertical rectangle
  ("."  (insert-char ?ë°ø) "ë°ø") ; curved double quotes close
  (","  (insert-char ?ë°æ) "ë°æ") ; curved double quotes close
 ; ("/"  (insert-char ?ë°æë°ø) "ë°æë°ø") ; curved double quotes close
  ("q" nil "cancel" nil)
))
...
#+BEGIN_SRC emacs-lisp :results none
(setq dired-recursive-copies (quote always)) ; ë°∞alwaysë°± means no asking
(setq dired-recursive-deletes (quote top)) ; ë°∞topë°± means ask once
#+END_SRC
...
The app is chosen from your OS's preference."
  ...
  (let* (
         ...
         (ÜÓdo-it-p (if (<= (length ÜÓfile-list) 5)
                       t
    ...
    (when ÜÓdo-it-p
      (cond
       ((string-equal system-type "windows-nt")
        (mapc
         (lambda (fPath)
           (w32-shell-execute "open" (replace-regexp-in-string "/" "\\" fPath t t))) ÜÓfile-list))
       ((string-equal system-type "darwin")
        (mapc
         (lambda (fPath) (shell-command (format "open \"%s\"" fPath)))  ÜÓfile-list))
       ((string-equal system-type "gnu/linux")
        (mapc
         (lambda (fPath) (let ((process-connection-type nil)) (start-process "" nil "xdg-open" fPath))) ÜÓfile-list))))))
#+END_SRC
...
(defun z/dired-open-in-desktop ()
  ...
  (cond
   ...
   ((string-equal system-type "gnu/linux")
    ...
    ;; (shell-command "xdg-open .") ;; 2013-02-10 this sometimes froze emacs till the folder is closed. úÙÛ÷ with nautilus
    ) ))
#+END_SRC
